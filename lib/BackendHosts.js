...
BackendHosts.prototype.fakeAdd = function (host, cb) {
  removeItemsFromArray(self.fakeAdded, hosts);
  removeItemsFromArray(self.fakeRemoved, hosts); // ensures no duplicates
  self.fakeAdded = self.fakeAdded.concat(hosts);
  return cb();
};
...
BackendHosts.prototype.fakeRemAll = function (hosts, cb) {
  removeItemsFromArray(self.fakeAdded, hosts);
  removeItemsFromArray(self.fakeRemoved, hosts); // ensures no duplicates
  self.fakeRemoved = self.fakeRemoved.concat(hosts);
  return cb();
};
